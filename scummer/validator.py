...
class Validator:
    ...
    def _check_schema(self,schema,data):
        if schema is None:
            return
        for key in schema:
            self._check_key(data=data, key=key, definition=schema[key])


    def _check_key(self, data, key, definition):
        if isinstance(definition, tuple):  # (<Meta>, <Param>)
            self._check_form(meta=definition[0], param=definition[1], data=data, key=key)
        elif isinstance(definition, list): # [<Meta>,(<Meta>, <Param>),...]
            flag = True
            key_name = key
            for i in definition:
                try:
                    self._check_key(data=data, key=key, definition=i)
                    flag = False
            ...
            self._check_form(meta=definition, data=data, key=key)

    def _check_form(self, data, key, meta, param={}):
        ...
        if isinstance(meta, str):  # TypeStr
            if meta == 'array':
                ...
                self._check_map(value=data[key], key_name=key_name, definition=param['definition'])
            else:
    ...
    def _check_map(self, value, key_name, definition):
        try:
            for k in value:
                self._check_key(data=value, key=k, definition=definition)
